...
const LandingPageV2 = () => {
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:5000'}/api/upload`, {
        method: 'POST',
        body: formData
      });
  ...
  return (
      <div style={{
        ...
        <header className={`app-header ${isScrolled ? 'scrolled' : ''}`}>
          <nav className="app-nav">
            ...
            <div className="nav-links">
              ...
              {isAuthenticated ? (
                  <>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', backgroundColor: theme.colors.hover, borderRadius: '20px' }}>
                      <span style={{ fontSize: '1rem' }}>👤</span>
                      <span style={{ color: theme.colors.text, fontSize: '0.9rem', fontWeight: '500' }}>{user?.name || user?.email?.split('@')[0]}</span>
                    </div>
                    <button onClick={logout} style={{ padding: '0.6rem 1.5rem', backgroundColor: 'transparent', color: '#ef4444', border: '2px solid #ef4444', borderRadius: '8px', cursor: 'pointer', fontSize: '0.9rem', fontWeight: '600' }}>Sign Out</button>
                  </>
              ) : (
                  <>
                    <button onClick={() => navigate('/login')} style={{ padding: '0.6rem 1.5rem', backgroundColor: 'transparent', color: theme.colors.primary, border: 'none', cursor: 'pointer', fontSize: '0.9rem', fontWeight: '600' }}>Sign In</button>
                  ...
                  <div
                      ...
                      style={{
                        ...
                        boxShadow: activeFeature === index
                            ? `0 10px 25px rgba(${theme.isDarkMode ? '249, 115, 22' : '59, 130, 246'}, 0.2)`
                            : '0 5px 15px rgba(0, 0, 0, 0.05)',
                        cursor: 'pointer'
                      }}
                  >
                    <div style={{ fontSize: '2.5rem', marginBottom: '1rem' }}>{feature.icon}</div>
                    <h3 style={{ fontSize: '1.25rem', fontWeight: '600', color: theme.colors.text, marginBottom: '0.75rem' }}>{feature.title}</h3>
                    <p style={{ color: theme.colors.textSecondary, lineHeight: '1.6', marginBottom: '1rem' }}>{feature.description}</p>
                    <p style={{ fontSize: '0.85rem', color: theme.colors.primary, fontWeight: '500' }}>{feature.detail}</p>
                  </div>
              ))}
            </div>
          </div>
        ...
        <section id="how-it-works" style={{ padding: '5rem 2rem', backgroundColor: theme.colors.background }}>
          <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
            <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
              ...
              {[
                ...
                { step: '3', title: 'Get Results', desc: 'Review simplified text, highlighted clauses, and ask questions via chat', icon: '✅' }
                  ...
                  <span style={{
                    ...
                    <h3 style={{ fontSize: '1.25rem', fontWeight: '600', color: theme.colors.text, marginBottom: '0.75rem' }}>{item.title}</h3>
                    <p style={{ color: theme.colors.textSecondary, lineHeight: '1.6' }}>{item.desc}</p>
                  </div>
              ))}
            </div>
          </div>
